...
var charsets: string = `<span value="" title="與資料庫相同" selected>默認</span>
...
export function activate(context: vscode.ExtensionContext) {
	...
	let addLinkView: vscode.WebviewPanel | undefined = undefined;
	let editLinkView: vscode.WebviewPanel | undefined = undefined;
	let addTableView: vscode.WebviewPanel | undefined = undefined;
	let editTableView: vscode.WebviewPanel | undefined = undefined;
	...
	commands.push(vscode.commands.registerCommand('veldb.viewTable', (actor) => {
		...
		view.webview.onDidReceiveMessage(msg=>{
			if(msg.command == "save"){
				...
				queryMutilSql(actor.server,adds,(res: any)=>{
				...
				queryMutilSql(actor.server,updates,(res: any)=>{
					if(res.affectedRows == 0){
				...
				queryMutilSql(actor.server,dels,(res: any)=>{
					if(res.affectedRows == 0){
...
function getView(viewname: string,extendurl: vscode.Uri,args?: any[]): string {
	let viewcontent = "";
	switch (viewname) {
		case "addLink":
...
export class veldatabaseProvider implements vscode.TreeDataProvider<Dependency> {
	...
	public query(element: Dependency): Promise<Dependency[]> {
...
function querySql(server: mysql.Pool,sql: string): Promise<any[]> {
	return new Promise((resolve,reject)=>{
...
async function queryMutilSql(server: mysql.Pool, sqls: Array<string>, callback?:Function) {
...
export class Dependency extends vscode.TreeItem {
	...
	constructor(
		...
		private readonly des: string,
		public readonly collapsibleState: vscode.TreeItemCollapsibleState,
		public readonly icon: string,
		public readonly server: mysql.Pool,
		public readonly dbname: string,
		public readonly tbname: string,
		public readonly tag: string,
		public readonly serverid?:string | number,
...
export class ServerList extends Object {
	...
	public server: mysql.Pool = mysql.createPool("");
}